...
Answer an individual ESG reporting question with an evidence-grounded factual narrative whose length follows the question and supplied evidence. Framework guidance may inform the reasoning, but the output is only the qualitative ESG answer for the requested field, not a full report section.
...
- Let answer length follow the question and available evidence. Cover every directly supported facet needed for a complete answer, but do not add repetition, generic ESG language, or unsupported context. Use a short list only when the question itself asks for several distinct items.
